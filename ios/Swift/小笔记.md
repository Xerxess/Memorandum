...
- 在使用 protocol 如 let sub:TestProtocol = subClass() 上下文中查找方法只会在实现的协议的类型中查找，不会在继承的类中查找
- 其他情况

```swift
protocol TestProtocol {
    func method(_ value: Any?)
}

class supClass:TestProtocol {
    var index = 0
    func method(_ value: Any?){
        print("supClass_Any?")
        index += 1
    ...
    func method(_ value: Double){
              print("supClass_Double")
          }
   
}

class subClass:supClass {
    func method(_ value: String){
                 print("subClass_String")
             }
    func method(_ value: Float){
                     print("subClass_Float")
                 }
}

let sub:TestProtocol = subClass()
sub.method("String")
// supClass_Any?
// supClass_Any?
// supClass_Any?
