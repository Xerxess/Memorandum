...
- [API](#api)
    ...
    - [Adding a custom transition or presentation 添加自定义过渡或演示文稿](#adding-a-custom-transition-or-presentation-添加自定义过渡或演示文稿)
    - [Adapting to environment changes  适应环境变化](#adapting-to-environment-changes--适应环境变化)
    - [Adjusting the interface style 调整界面样式](#adjusting-the-interface-style-调整界面样式)
    - [Managing child view controllers in a custom container 在自定义容器中管理子视图控制器](#managing-child-view-controllers-in-a-custom-container-在自定义容器中管理子视图控制器)
    - [Responding to containment events 应对遏制事件](#responding-to-containment-events-应对遏制事件)
    - [Getting other related view controllers 获取其他相关视图控制器](#getting-other-related-view-controllers-获取其他相关视图控制器)
    - [Configuring a navigation interface 配置导航界面](#configuring-a-navigation-interface-配置导航界面)
    - [Configuring tab bar content 配置标签栏内容](#configuring-tab-bar-content-配置标签栏内容)
    - [Working with scrolling content 处理滚动内容](#working-with-scrolling-content-处理滚动内容)
    - [Supporting app extensions 支持应用程序扩展](#supporting-app-extensions-支持应用程序扩展)
    - [Coordinating with system gestures 与系统手势协调](#coordinating-with-system-gestures-与系统手势协调)
    - [Working with focus 专注地工作](#working-with-focus-专注地工作)
    - [Managing pointer lock state 管理指针锁定状态](#managing-pointer-lock-state-管理指针锁定状态)
    - [Managing the status bar 管理状态栏](#managing-the-status-bar-管理状态栏)
    - [Managing the Touch Bar 管理触控栏](#managing-the-touch-bar-管理触控栏)
    - [Accessing the available key commands 访问可用的键盘命令](#accessing-the-available-key-commands-访问可用的键盘命令)
    - [Adding editing behaviors to your view controller 将编辑行为添加到视图控制器中](#adding-editing-behaviors-to-your-view-controller-将编辑行为添加到视图控制器中)
    - [Handling memory warnings 处理内存警告](#handling-memory-warnings-处理内存警告)
    - [Managing state restoration 管理恢复](#managing-state-restoration-管理恢复)
    - [Supporting Swift Playgrounds](#supporting-swift-playgrounds)
    - [Logging user interaction intervals 记录用户交互间隔](#logging-user-interaction-intervals-记录用户交互间隔)
